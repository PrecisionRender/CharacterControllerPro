...
namespace Game
{
    ...
    public class FootIK : Script
    {
        ...
        {
            if (Physics.SphereCast(new Vector3(leftFootIK.Position.X, Actor.Position.Y + 30, leftFootIK.Position.Z), 3, Vector3.Down, out RayCastHit leftFootHit, 80, collisionMask))
            {
            ...
            if (Physics.SphereCast(new Vector3(rightFootIK.Position.X, Actor.Position.Y + 30, rightFootIK.Position.Z), 3, Vector3.Down, out RayCastHit rightFootHit, 80, collisionMask))
            {
                // Get a forward-facing direction relative to the ground noraml
