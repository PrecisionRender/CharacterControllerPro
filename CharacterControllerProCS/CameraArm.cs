...
namespace Game
{
    ...
    public class CameraArm : Script
    {
        ...
        {
            // Cast sphere
            if (ShouldUseCollision && Physics.SphereCast(Actor.Position, CollisionSphereRadius, Transform.Backward, out RayCastHit hit, ArmLength, ArmCollisionMask))
            {
                // If sphere hits something, move camera out of collided body
                _camera.Position = Actor.Position + Transform.Backward * ArmLength * (hit.Distance / ArmLength);
            }
            else
            {
